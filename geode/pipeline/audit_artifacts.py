...
def _write_api_verification(root: Path, now: datetime, api_base_url: str | None) -> str:
    ...
    app_file = root / "geode" / "api" / "app.py"
    for api_path in DATA_API_PATHS:
        capture = {
            "path": api_path,
            "handler_file": app_file.relative_to(root).as_posix() if app_file.exists() else None,
            "handler_file_exists": app_file.exists(),
            "request_url": None,
            "http_status": None,
    ...
    path = root / AUDIT_REPORT_DIR / f"DATA_API_VERIFICATION_{AUDIT_DATE}.json"
    atomic_write_json(
        path,
        {
            "generated_at": now.isoformat(),
            ...
            "captures": captures,
            "boundary": "Backend handler presence is not the same as a successful live request.",
        },
        root,
    )
    return path.relative_to(root).as_posix()
...
| verified | source-backed or locally validated | Data reviewer | Evidence path, hash, or validation output exists | Avoid using verified for unaudited output or external-source state. |
