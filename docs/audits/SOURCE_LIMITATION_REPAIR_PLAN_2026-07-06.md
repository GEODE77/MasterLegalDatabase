...
2. Use `python -m geode.pipeline.legiscan_source_finder_checklist --root . --write` to keep the source-finding checklist current.
3. Work the 41-item modern repair queue first using `python -m geode.pipeline.legiscan_repair_intake` after each official replacement file is verified.
4. Track reviewer and open-item progress with `python -m geode.pipeline.legiscan_repair_progress_dashboard --root . --write`.
5. Build a host-specific recovery workflow for the large legacy archive group.
6. Keep the main download audit at warning level until the coverage gap is either repaired or formally accepted by the project owner.
